...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _onImageButtonPressed(ImageSource source) async {
    ...
    if (isVideo) {
      _picker.getVideo(source: source).then((PickedFile file) {
      ...
      try {
        var result = await _picker.getImage(source: source);
      ...
      if (response.file != null) {
        setState(() {
          if (response.type == RetrieveType.video) {
            isVideo = true;
            _controller = VideoPlayerController.file(File(response.file.path))
              ..addListener(_onVideoControllerUpdate)
              ..setVolume(1.0)
              ..initialize()
              ..setLooping(true)
